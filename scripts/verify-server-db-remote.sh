...
# Script to verify promotional content tables on backend server database
# Executes verification commands via SSH

SERVER_IP="116.118.48.209"
SERVER_USER="root"

echo "🔍 Connecting to backend server to verify promotional content tables..."
echo "📡 Server: $SERVER_USER@$SERVER_IP"
echo ""

# Execute verification commands on the server
...
if ! docker ps | grep -q "icfix-postgres"; then
    ...
    echo "   Make sure docker-compose-prod.yml is running"
    exit 1
fi
...
# List all tables and filter for promotional content
echo "📋 Listing promotional content tables..."
...
if docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\d promotional_banner" > /dev/null 2>&1; then
    echo "   ✓ Table exists"
    COUNT=$(docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT COUNT(*) FROM promotional_banner;" 2>/dev/null | xargs)
    echo "   📊 Records: $COUNT"
    echo "   📐 Structure:"
    docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\d promotional_banner" 2>/dev/null | head -20
else
...
if docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\d service_feature" > /dev/null 2>&1; then
    echo "   ✓ Table exists"
    COUNT=$(docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT COUNT(*) FROM service_feature;" 2>/dev/null | xargs)
    echo "   📊 Records: $COUNT"
    echo "   📐 Structure:"
    docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\d service_feature" 2>/dev/null | head -15
else
    echo "   ❌ Table does NOT exist"
fi
echo ""
...
if docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\d testimonial" > /dev/null 2>&1; then
    echo "   ✓ Table exists"
    COUNT=$(docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT COUNT(*) FROM testimonial;" 2>/dev/null | xargs)
    echo "   📊 Records: $COUNT"
    echo "   📐 Structure:"
    docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\d testimonial" 2>/dev/null | head -15
else
    echo "   ❌ Table does NOT exist"
fi
echo ""
...
if docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\d homepage_section" > /dev/null 2>&1; then
    echo "   ✓ Table exists"
    COUNT=$(docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT COUNT(*) FROM homepage_section;" 2>/dev/null | xargs)
    echo "   📊 Records: $COUNT"
    echo "   📐 Structure:"
    docker exec icfix-postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\d homepage_section" 2>/dev/null | head -15
else
    echo "   ❌ Table does NOT exist"
fi
echo ""
