-- Final Products Setup - Direct INSERT approach
-- Uses explicit variant IDs and creates everything step by step

BEGIN;

-- Step 1: Create 4 price sets if needed
DO $$
DECLARE
  ps_count int;
BEGIN
...
-- Step 2: Link variants to price sets using explicit variant IDs
-- iPhone variant
...
SELECT 
  ...
  NOW(),
  NOW()
WHERE NOT EXISTS (SELECT 1 FROM product_variant_price_set WHERE variant_id = 'dfd33cb6-83e5-49ec-a82a-fc59d71ce90a');
...
SELECT 
  ...
  NOW(),
  NOW()
WHERE NOT EXISTS (SELECT 1 FROM product_variant_price_set WHERE variant_id = '84df59d3-ad1e-4bd8-a2d8-bab9a3613522');
...
SELECT 
  ...
  NOW(),
  NOW()
WHERE NOT EXISTS (SELECT 1 FROM product_variant_price_set WHERE variant_id = 'e8b58a38-765d-41de-a78c-97437262cc8e');
...
SELECT 
  ...
  NOW(),
  NOW()
WHERE NOT EXISTS (SELECT 1 FROM product_variant_price_set WHERE variant_id = '2ee552b3-f1d9-474d-9186-3dd50b73d2ee');

-- Step 3: Create prices for each linked price set
...
SELECT 
  ...
  CASE 
    WHEN ii.sku = 'IPHONE15PRO-128-NAT' THEN 100
    WHEN ii.sku = 'MBA-M3-8-256-MID' THEN 50
    ELSE 200
  END,
  0,
  CASE 
